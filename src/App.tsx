...
const singleSignerNetworks = allNetworks.map(network => {
	...
	network.relayer = {
		...
		provider: {
			url: env === 'dev' ? `https://dev-nodes.sequence.app/${network.name}` : `https://nodes.sequence.app/${network.name}`,
		},
	};
...
function App() {
	...
	async function runTransactionsParallel() {
		...
		try {
			...
			console.log('singleSignerNetworks', singleSignerNetworks);

			const session = await Session.singleSigner({
				signer: privateKey,
				projectAccessKey,
				settings: {
					networks: singleSignerNetworks,
				},
			});
			...
			await Promise.allSettled(
				tasks.map(async i => {
					...
					try {
						...
						setResults(prev => prev.map(r => (r.index === i ? {...r, endedAtMs, durationMs, hash} : r)).sort((a, b) => a.index - b.index));
					} catch (err: unknown) {
						const endedAtMs = Date.now();
						const durationMs = endedAtMs - startedAtMs;
						const error = err instanceof Error ? err.message : String(err);
						setResults(prev => prev.map(r => (r.index === i ? {...r, endedAtMs, durationMs, error} : r)).sort((a, b) => a.index - b.index));
					}
				}),
			);
		} catch (err: unknown) {
	...
	return (
		<div style={{maxWidth: 900, margin: '0 auto', padding: 16}}>
			...
			<p style={{marginTop: -8}}>Uses Sequence session auth to relay N parallel transactions to a target address.</p>

			<div className="card" style={{display: 'grid', gap: 12}}>
				<label>
					<span>Network</span>
					<select value={selectedChainId} onChange={e => setSelectedChainId(Number(e.target.value))} disabled={isRunning} style={{display: 'block', marginTop: 6}}>
				...
				<label>
					...
					<input type="text" placeholder="0x..." value={targetAddress} onChange={e => setTargetAddress(e.target.value)} disabled={isRunning} style={{width: '100%', marginTop: 6}} />
				</label>

				<label>
					<span>Number of transactions</span>
					<input type="number" min={1} step={1} value={txCount} onChange={e => setTxCount(Number(e.target.value))} disabled={isRunning} style={{width: '100%', marginTop: 6}} />
				</label>
				...
				{showAdvanced && (
					<div style={{display: 'grid', gap: 12, padding: 12, border: '1px solid #444', borderRadius: 8}}>
						<label>
							...
							<input type="text" placeholder={selectedNetwork.rpcUrl} value={customRpcUrl} onChange={e => setCustomRpcUrl(e.target.value)} disabled={isRunning} style={{width: '100%', marginTop: 6}} />
						</label>
						<label>
				...
				<div style={{display: 'flex', gap: 12}}>
					...
					<button onClick={() => setResults([])} disabled={isRunning}>
						Clear results
					</button>
				</div>
			...
			<p style={{marginTop: 24, fontSize: 12, opacity: 0.8}}>
				Parallel send approach adapted from sample implementation in{' '}
				<a href="https://github.com/0xsequence/tps-reporter/blob/main/src/index.ts" target="_blank" rel="noreferrer">
					tps-reporter
				</a>
				.
			</p>
